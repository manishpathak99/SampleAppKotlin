...
class ImageUtility(private val context: Context) {
    ...
    fun CameraGalleryIntent(cameraRequestCode: Int, galleryRequestCode: Int): Uri {
        ...
        val items = arrayOf<CharSequence>(context.resources.getString(R.string.camera), context.resources.getString(R.string.gallery))
        dialog.setItems(items) { d, n ->
            when (n) {
                0 -> {
                    val intent = Intent(MediaStore.ACTION_IMAGE_CAPTURE)
                    intent.putExtra(MediaStore.EXTRA_OUTPUT, outputFileUri)
                    (context as Activity).startActivityForResult(intent, cameraRequestCode)
                }
                1 -> {
                    val pickIntent = Intent(Intent.ACTION_PICK)
                    pickIntent.type = "image/*"
                    (context as Activity).startActivityForResult(pickIntent, galleryRequestCode)
                }
                else -> {
                }
            }
        }
        dialog.setTitle(context.resources.getString(R.string.selection))
        if (outputFileUri != // WHERE clause selection arguments (none)
                null) {
            dialog.show()
        }
    ...
    val file: File
        get() = File(Environment.getExternalStorageDirectory().toString() + File.separator + "PlowzMowz"
                + File.separator)
